...
spansion_s29gl064s_device::spansion_s29gl064s_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: intelfsh16_device(mconfig, SPANSION_S29GL064S, tag, owner, clock, 0x1000000, MFG_SPANSION, 0x227e)
{
	// senbbs
	m_sector_is_4k = false;
}
...
void intelfsh_device::write_full(uint32_t address, uint32_t data)
{
	...
	case FM_READAMDID2:
		...
		else if( ( address & 0xffff ) == 0x555 && ( data & 0xff ) == 0x20 )
		{
			//printf("spansion bypass1\n");
			m_flash_mode = FM_UNLOCKBYPASS1;
		}
		else
		{
	...
	case FM_CLEARPART1:
		...
		{
			//printf( "unexpected %02x in FM_CLEARPART1\n", data & 0xff );
			if ((data & 0xff) == 0x20 )
			{
				//printf("Unlock bypass1\n");
				m_flash_mode = FM_UNLOCKBYPASS1;
			}
		}
		break;
	case FM_UNLOCKBYPASS1:
	{
		if ( (data & 0xff) == 0xa0)
		{
			//printf("Unlock bypass2\n");
			m_flash_mode = FM_UNLOCKBYPASS2;
		}
		else if ( (data & 0xff) == 0x90)
		{
			//printf("Unlock bypass reset\n");
			m_flash_mode = FM_NORMAL;
		}
		break;
	}
	case FM_UNLOCKBYPASS2:
	{
		//printf("Unlock bypass program %08x %04x\n", address, data);
		m_data[address*2] = data >> 8;
		m_data[address*2+1] = data;
		m_flash_mode = FM_UNLOCKBYPASS1;
		break;
